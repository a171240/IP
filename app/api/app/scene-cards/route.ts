...
import {
  createAliyunRdsSceneCard,
  listAliyunRdsSceneCards,
...
export async function GET(request: NextRequest) {
  try {
    ...
    const cards = await listAliyunRdsSceneCards(auth.user.id, limit)
...
export async function POST(request: NextRequest) {
  try {
    ...
    const card = await createAliyunRdsSceneCard(auth.user.id, parsed.data)

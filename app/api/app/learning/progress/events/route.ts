...
export async function POST(request: NextRequest) {
  try {
    ...
    const result = await applyLearningProgressEvent(scope.scope, payload)
    if (!result.ok) {
    ...
    return learningProgressErrorResponse(error, "learning_progress_event_failed")
  }
}

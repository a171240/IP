...
type ScenarioName = "mock_done" | "no_env_failed" | "retry_cap_failed"
...
function startServer(cwd: string, port: number, env: NodeJS.ProcessEnv) {
  ...
  const logPrefix = `[video-e2e:${port}]`
...
async function seedDependencies(admin: ReturnType<typeof createClient>, userId: string) {
  ...
  const sourceInsert = await admin.from("content_sources").insert({
    ...
    status: "ready",
    title: "Video Matrix Source",
    text_content: "Matrix source text",
    images: [],
    meta: {},
    raw_payload: {},
  })
  ...
  const rewriteInsert = await admin.from("content_rewrites").insert({
    ...
    tone: "professional",
    constraints: {},
    result_title: "Matrix title",
    result_body: "这是用于视频任务矩阵测试的口播正文。",
    result_script: "这是用于视频任务矩阵测试的口播脚本。",
    status: "done",
  })
  ...
  const avatarInsert = await admin.from("store_profiles").insert({
    id: avatarProfileId,
    user_id: userId,
    name: "Matrix Avatar",
    boss_drive_video_path: "https://example.com/avatar.mp4",
...
async function runScenario(
  ...
  try {
    ...
    const seed = await seedDependencies(admin, auth.user_id)
    ...
    const createDuplicate = await httpJson(baseUrl, "/api/mp/video/jobs", {
      method: "POST",
      token: auth.access_token,
      body: requestBody,
    })
    ...
    const duplicateJobId = extractJobId(createDuplicate)
    ...
    return {
      ...
      same_job_id: Boolean(firstJobId && duplicateJobId && firstJobId === duplicateJobId),
      job_id: firstJobId,
      timeline,
      final_get: finalGet,
      final_db: finalDb,
...
async function run() {
  ...
  const mockVideoUrl =
    unwrap(String(baseEnv.VIDEO_PIPELINE_MOCK_VIDEO_URL || "")) ||
    "https://commondatastorage.googleapis.com/gtv-videos-bucket/sample/BigBuckBunny.mp4"

  const scenarioList: ScenarioConfig[] = [
    {
      name: "mock_done",
      port: Number(process.env.VIDEO_E2E_MOCK_PORT || 3320),
      requireVolcSpeech: true,
      envOverride: {
        VIDEO_PIPELINE_MOCK_VIDEO_URL: mockVideoUrl,
        VIDEO_PIPELINE_ALLOW_AUDIO_ONLY_SUCCESS: "",
      },
    },
    {
      name: "no_env_failed",
      port: Number(process.env.VIDEO_E2E_NOENV_PORT || 3321),
    ...
    },
    {
      ...
      port: Number(process.env.VIDEO_E2E_RETRY_PORT || 3322),
      requireVolcSpeech: true,
      envOverride: {
        VIDEO_PIPELINE_MOCK_VIDEO_URL: "",
        VIDEO_PIPELINE_ALLOW_AUDIO_ONLY_SUCCESS: "",
  ...
  const outputPath =
    process.env.VIDEO_E2E_MATRIX_OUTPUT || path.join(cwd, `tmp/video-e2e-matrix-${Date.now().toString()}.json`)
  ...
  console.log(`matrix_output=${outputPath}`)
  console.log(JSON.stringify(payload, null, 2))
}

run().catch((error) => {
  const message = error instanceof Error ? error.message : String(error)
  console.error(`video_e2e_matrix_failed:${message}`)
  process.exit(1)
})

...
function renderMarkdown(audit) {
  ...
  return [
    ...
    `- backendEnvImportBatches: blocked ${backendEnvImportBatches.blockedCredentialCount || 0}, ready secret env ${backendEnvImportBatches.readySecretEnvVariableCount || 0}, groups ${backendEnvImportBatches.readySecretEnvVariableGroupCount || 0}, import batches ${backendEnvImportBatches.summary?.importBatchCount || 0}`,
    ...
    `- compatibilityReviewChecklistItemCount: ${rdsMigrationPackage.summary.compatibilityReviewChecklistItemCount ?? 0}`,
    `- compatibilityCategories: ${(rdsMigrationPackage.compatibilityReview?.categories || []).join(", ") || "none"}`,
    `- compatibilityChecklistCodes: ${(rdsMigrationPackage.compatibilityReviewChecklist || []).map((item) => item.code).join(", ") || "none"}`,
    ...
    `- credentialAcquisitionQueueScope: ${envHandoff.credentialAcquisitionQueue?.queueScope || "unknown"}`,
    `- credentialAcquisitionQueueMissingNames: ${envHandoff.credentialAcquisitionQueue?.missingCredentialNames?.join(", ") || "none"}`,
    `- credentialAcquisitionQueueActionIds: ${envHandoff.credentialAcquisitionQueue?.items?.map((item) => item.actionId).join(", ") || "none"}`,
    "",
    ...
    `- importBatchCount: ${backendEnvImportBatches.summary?.importBatchCount || 0}`,
    `- blockedSecretImportBatchCount: ${backendEnvImportBatches.summary?.blockedSecretImportBatchCount || 0}`,
    `- readySecretImportBatchCount: ${backendEnvImportBatches.summary?.readySecretImportBatchCount || 0}`,
...
function main() {
  ...
  console.log(JSON.stringify({
    ...
    envHandoff: {
      ...
      credentialAcquisitionQueueScope: envHandoff.credentialAcquisitionQueue?.queueScope || "",
      credentialAcquisitionQueueMissingNames: envHandoff.credentialAcquisitionQueue?.missingCredentialNames || [],
      credentialAcquisitionQueueActionIds: (envHandoff.credentialAcquisitionQueue?.items || []).map((item) => item.actionId),
    },
    backendEnvImportBatches: {
      ...
      blockedSecretImportBatchCount: backendEnvImportBatches.summary?.blockedSecretImportBatchCount || 0,
      readySecretImportBatchCount: backendEnvImportBatches.summary?.readySecretImportBatchCount || 0,
      importBatchIds: backendEnvImportBatches.summary?.importBatchIds || [],
    ...
    rdsMigrationPackage: {
      ...
      compatibilityChecklistCodes: (rdsMigrationPackage.compatibilityReviewChecklist || []).map((item) => item.code),

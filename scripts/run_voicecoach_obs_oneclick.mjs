...
const GATE_THRESHOLDS = Object.freeze({
  ...
  G1: Object.freeze({
    ...
    optional_stage_metrics: OPTIONAL_STAGE_KEYS,
...
function validateBenchReport(file, { flashRequired }) {
  ...
  return {
    ...
    supports_audio_chunk: rounds.some((round) =>
      Array.isArray(round?.events) ? round.events.some((ev) => ev?.type === "customer.audio_chunk") : false,
    ),
  }
}

...
const jobName = `meiye-revoke-schema-${suffix.slice(-10)}`
const eventId = `auth-revoke-schema-${suffix}`
...
function printHelp() {
  console.log([
    "Usage:",
    `  ${ALLOW_ENV}=1 node scripts/apply-production-cn-rds-auth-revocation-schema-via-sae-job.mjs --execute`,
    "",
    "Scope:",
    ...
    "  - applies schema-only SQL for public.app_auth_token_revocations",
...
function main() {
  ...
  try {
    ...
    console.log(`[sae-auth-revocation-schema] run=${suffix}`)
    console.log(`[sae-auth-revocation-schema] schemaSha256=${plan.schema.sha256}`)
    ...
    if (args.skipCleanup) {
      console.log("[sae-auth-revocation-schema] cleanup skipped by flag")
    } else {
      cleanup()
    }
  }
...
function createJobTemplate(config) {
  console.log("[sae-auth-revocation-schema] creating temporary SAE job template")
  ...
  console.log(`[sae-auth-revocation-schema] jobAppId=${appId}`)
  return appId
}

function execSchemaJob(databaseUrl, schemaBase64) {
  console.log("[sae-auth-revocation-schema] executing schema job")
  const commandArgs = JSON.stringify(["-e", buildRunnerScript()])
  const envs = JSON.stringify([
    { name: "DATABASE_URL_CN", value: databaseUrl },
    { name: "MEIYE_AUTH_REVOCATION_SCHEMA_B64", value: schemaBase64 },
  ])
  const cliArgs = [
    "sae", "ExecJob",
    "--region", REGION,
  ...
  for (let attempt = 1; attempt <= 30; attempt += 1) {
    try {
      ...
      if (attempt % 3 === 0) {
        console.log(`[sae-auth-revocation-schema] template change order still running elapsed=${attempt * 10}s`)
      }
      sleep(10_000)
    }
  }
  ...
  if (jobId) console.log(`[sae-auth-revocation-schema] jobId=${jobId}`)
  return jobId
}
...
async function main() {
  ...
  const schemaBase64 = String(process.env.MEIYE_AUTH_REVOCATION_SCHEMA_B64 || "").trim();
  ...
  if (!/create table if not exists public\\.app_auth_token_revocations/i.test(schemaSql)) {
    throw new Error("schema_sql_missing_revocations_table");
  }
  const client = new Client({
    connectionString: databaseUrl,
    application_name: "meiye-auth-revocation-schema-job",
  ...
  try {
    ...
    const result = await client.query(
      "select to_regclass('public.app_auth_token_revocations')::text as table_name, " +
      "(select count(*)::int from pg_indexes where schemaname='public' and tablename='app_auth_token_revocations') as index_count"
    );
    const row = result.rows[0] || {};
    if (row.table_name !== "app_auth_token_revocations" && row.table_name !== "public.app_auth_token_revocations") {
      throw new Error("revocation_table_not_found_after_schema_apply");
    }
    if (Number(row.index_count || 0) < 3) throw new Error("revocation_index_count_too_low");
    console.log("SCHEMA_APPLY_RESULT=" + JSON.stringify({
      ok: true,
      table: "public.app_auth_token_revocations",
...
function waitForJobSuccess(appId, jobId) {
  for (let attempt = 1; attempt <= 90; attempt += 1) {
    ...
    if (attempt % 6 === 0) {
      console.log(`[sae-auth-revocation-schema] job status=${normalized.label} elapsed=${attempt * 10}s`)
    }
    sleep(10_000)
  }
...
function cleanup() {
  console.log("[sae-auth-revocation-schema] cleanup started")
  ...
  console.log("[sae-auth-revocation-schema] cleanup finished")
}
...
function safeRun(command, cliArgs) {
  ...
  if (result.status !== 0) {
    ...
    console.log(`[sae-auth-revocation-schema] cleanup warning: ${command} ${cliArgs.slice(0, 4).join(" ")}: ${output}`)
    return false
  }
  return true
}

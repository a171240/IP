...
async function run() {
  ...
  for (let i = 0; i < beauticianLines.length; i++) {
    ...
    round.events = polled.events.map((ev) => ({
      ...
      first_audio_chunk_ms: toFiniteNumber(ev.data?.first_audio_chunk_ms),
      first_audio_play_ms: toFiniteNumber(ev.data?.first_audio_play_ms),
      inline_audio_eligible:
        typeof ev.data?.inline_audio_eligible === 'boolean' ? ev.data.inline_audio_eligible : null,
      submit_fastpath_hit:
        typeof ev.data?.submit_fastpath_hit === 'boolean' ? ev.data.submit_fastpath_hit : null,
      asr_input_source: ev.data?.asr_input_source || null,
      realtime: typeof ev.data?.realtime === 'boolean' ? ev.data.realtime : null,
      audio_chunk_seq: toFiniteNumber(ev.data?.seq),
      audio_chunk_duration_ms: toFiniteNumber(ev.data?.duration_ms),
      audio_chunk_final: typeof ev.data?.is_final === 'boolean' ? ev.data.is_final : null,

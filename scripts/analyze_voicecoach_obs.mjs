...
function buildStageMetrics(rounds, events) {
  ...
  const audioChunks = events.filter((ev) => ev?.type === "customer.audio_chunk")
  ...
  return {
    ...
    first_audio_chunk_ms: stageStats(
      audioChunks.map((ev) => ev?.first_audio_chunk_ms ?? ev?.stage_elapsed_ms ?? null),
    ),
    first_audio_play_ms: stageStats(audioReady.map((ev) => ev?.first_audio_play_ms)),
...
function summarizeGroup(label, bench, opts) {
  ...
  const inlineAudioEligibleValues = asrReady
    .map((ev) => (typeof ev?.inline_audio_eligible === "boolean" ? ev.inline_audio_eligible : null))
    .filter((item) => item !== null)
  const inlineAudioEligibleRate =
    inlineAudioEligibleValues.length > 0
      ? Number((inlineAudioEligibleValues.filter(Boolean).length / inlineAudioEligibleValues.length).toFixed(4))
      : null

  const submitFastpathValues = textReady
    .map((ev) => (typeof ev?.submit_fastpath_hit === "boolean" ? ev.submit_fastpath_hit : null))
    .filter((item) => item !== null)
  const submitFastpathHitRate =
    submitFastpathValues.length > 0
      ? Number((submitFastpathValues.filter(Boolean).length / submitFastpathValues.length).toFixed(4))
      : null

  const asrInputSourceDistribution = countBy(asrReady.map((ev) => ev?.asr_input_source || ""))
  const realtimeEventDistribution = countBy(
    events.map((ev) => (typeof ev?.realtime === "boolean" ? String(ev.realtime) : "")),
  )
  ...
  return {
    ...
    asr_input_source_distribution: asrInputSourceDistribution,
    realtime_event_distribution: realtimeEventDistribution,
    queue_wait_before_tts_invalid_rate: queueWaitBeforeTtsInvalidRate,
    tts_cache_hit_rate: ttsCacheHitRate,
    inline_audio_eligible_rate: inlineAudioEligibleRate,
    submit_fastpath_hit_rate: submitFastpathHitRate,
    stage_metrics: stageMetrics,
    counts: {
      asr_ready: asrReadyCount,
      text_ready: textReady.length,
      audio_chunk: audioChunks.length,

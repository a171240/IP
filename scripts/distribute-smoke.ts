...
const baseUrl = process.env.DISTRIBUTE_SMOKE_BASE_URL || "http://localhost:3000"
const outputPath = process.env.DISTRIBUTE_SMOKE_OUTPUT || "/tmp/distribute_smoke_output.json"
const keepUser =
  args.has("--keep-user") ||
...
async function main() {
  ...
  const email = `qa.distribute.${Date.now()}@example.com`
  ...
  const { data: created, error: createError } = await adminClient.auth.admin.createUser({
    ...
    password,
    email_confirm: true,
    user_metadata: { source: "distribute-smoke" },
  })
  ...
  const rewriteInsert = await adminClient
    ...
    .insert({
      ...
      result_title: "QA rewrite title",
      result_body: "qa rewrite body",
      result_script: "qa rewrite script",
      result_tags: ["qa", "distribute"],
      cover_prompts: ["qa cover"],
  ...
  const rewriteId = rewriteInsert.data.id as string
  ...
  assert(
    invalidData.details && typeof invalidData.details === "object",
    "invalid_content_id_details_missing",
  )
  ...
  const jobsRes = await adminClient
    ...
    .select("id,user_id,content_id,mode,schedule_at,status,error,created_at")
    .eq("user_id", userId)
    .in("id", jobIds)
  ...
  const contentStudioJs = fs.readFileSync(
    path.join(process.cwd(), "mini-program-ui/pages/content-studio/index.js"),
    "utf8",
  )
  const videoJobsJs = fs.readFileSync(path.join(process.cwd(), "mini-program-ui/pages/video-jobs/index.js"), "utf8")
  const videoJobsWxml = fs.readFileSync(
    path.join(process.cwd(), "mini-program-ui/pages/video-jobs/index.wxml"),
    "utf8",
  )

...
function buildDirectCoverPromptRequirements(plan: BeautyCoverVisualPlan) {
  return [
    ...
    "封面必须是可直接发布的高级小红书封面和高级护肤杂志封面视觉，不是App入口页，不是引流落地页。",
    ...
    "版式必须有设计骨架：主标题区、副标题区、辅助信息点区、主视觉区；兼容旧称为标题区、主视觉区、短标签区。若提示词后续包含【辅助信息点】，必须把这些短点做成清晰可读的小标签、侧边短清单或分区信息条。",
...
export function normalizeCoverAsset(opts: {
  ...
  const promptBody = [
    ...
    `构图：${plan.composition}`,
    "文字要自然融入海报，而不是贴在模板上。",
    "版面至少有三层信息：主标题、短副标题、短标签/短清单；标题区、主视觉区、短标签区要清楚；标题要大而稳，短标签只做轻量辅助；不要生成单调的“背景图 + 大标题”。",

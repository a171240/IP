...
export type BailianSubmitResult = {
  provider: "bailian"
  model: string
  taskId: string
  taskStatus: string
...
export function isBailianAsrConfigured() {
  return Boolean(getBailianApiKey())
}

export function getBailianAsrModel() {
...
function transcriptionParameters() {
  const model = getBailianAsrModel()
...
export async function submitBailianAsrTask(opts: {
  audioUrl: string
}): Promise<BailianSubmitResult> {
  const model = getBailianAsrModel()
...
export async function queryBailianAsrTask(taskId: string): Promise<BailianPollResult> {

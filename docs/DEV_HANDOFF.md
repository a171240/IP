...
- 积分展示：P7/P8 页已显示「预计扣积分/剩余积分」。
- 发布慢（手机端 60s 超时）体验修复：后端发布不阻塞上传；小程序端超时后自动轮询草稿状态拿二维码。
- 数据库迁移（必须）：`supabase/migrations/20260206_add_xhs_drafts.sql`（新环境需执行）。
- 回归脚本：`node scripts/regress-mp-p7p8-topics.js`（2026-02-06 已通过）。

...
`aliyun:app-api:bridge-map` 是小程序链路复用门禁：它读取 `deploy/app-api-production-cn.bridge-map.json`，逐条校验 32 个 APP API route、App route 文件、源小程序 API 文件和小程序源页面是否一致。当前分类是 26 条 `app_native`、2 条 `mp_reexport`、2 条 `app_alias`、2 条 `native_health`。微信登录必须保持 `app_native/app_alias`，不能回退复用小程序 `wx.login` 链路；`/api/app/profile` 已切到 APP-native RDS repository，`/api/app/entitlements` 作为 APP alias 复用该 RDS 链路；服务记录长录音 12 条、店长看板 3 条、门店邀请 4 条 APP API 已切到 RDS/OSS/ASR 或 RDS 原生链路。
...
`corepack pnpm aliyun:env:handoff` 会把 APP production-cn 的 63 个环境变量整理成无值获取与导入手册，按后端必填阻塞、APP 发布/AASA 阻塞、ready plain env、ready secret env 和可后置变量分组，回答“从哪里取得、写到阿里云哪里、当前是否阻塞、禁止写到哪里”。`corepack pnpm aliyun:operator:tasks`、`corepack pnpm aliyun:status`、`corepack pnpm aliyun:sensitive:blockers` 和 `corepack pnpm aliyun:operator:handoff` 还会额外输出 `sensitiveActionItems`，专门回答“还需要用户介入哪些密钥、密码、token、付款或受控标识符动作”。该字段只列变量名、控制台路径、动作、解除条件和禁止事项，不输出任何 value。当前会把微信开放平台移动应用 AppID/AppSecret、Apple Team ID、ACR 企业版付费确认、ACR/SAE 镜像拉取认证、OSS RAM Secret 或 STS 注入、RDS PostgreSQL 的 `DATABASE_URL_CN` 与迁移验收、以及本地已有 ready 值但尚未导入阿里云的敏感/连接类变量组分开列出；其中 `WECHAT_OPEN_APP_ID` 是服务端标识符，导入阿里云 SAE plain env，`WECHAT_OPEN_APP_SECRET` 才走 KMS/Secrets Manager/SAE secret env。`ALIYUN_OSS_SECURITY_TOKEN` 只是选择临时 STS 凭证时才需要的可选 token；如果采用受限 RAM AccessKey 或 SAE 运行时角色链路，它不属于当前 backend-only 硬缺 credential。`aliyun:sensitive:blockers` 会额外执行 secret-like 输出扫描，只有在报告不含疑似密钥值时才通过；`aliyun:sensitive:blockers:backend` 和 `aliyun:release:artifacts:backend` 的 `sensitive-blockers.json/md` 使用 backend-only 口径，只保留 ACR 付费、ACR/SAE 镜像认证、OSS RAM/STS、RDS `DATABASE_URL_CN` 和 ready secret env 导入，当前 blocked credential name 应为 `DATABASE_URL_CN`，微信开放平台、Apple Team ID 和 Android release signing 留到 APP 发布阶段。`corepack pnpm aliyun:resources:matrix` 则回答“阿里云上到底要开通/确认哪些资源”：它只列资源项、控制台路径、非密钥字段、当前 blocker 和验收命令，不输出任何 value，也不会执行创建、付款、DNS 修改、镜像推送或部署。
...
`corepack pnpm aliyun:rds:migration:plan` 当前证明 APP 第一版 25 条 RDS 必需业务路由已经全部指向 `DATABASE_URL_CN` 支撑的 APP-native RDS repository，首版 Supabase 业务数据访问剩余为 0；完整 APP API 仍有 4 条延期范围路由保留 Supabase 业务数据访问。迁移清单见 `docs/app-production-cn-rds-migration-plan.md`。`REDIS_URL_CN` 仍只有在 production-cn 队列/缓存实现明确依赖 Tair/Redis 时才升级为必填阻塞项。
...
corepack pnpm aliyun:app-api:bridge-map（32 mapped routes / sourceTypes: app_native 26, mp_reexport 2, app_alias 2, native_health 2）
corepack pnpm aliyun:app-client:contract（40 audited calls / 34 unique client routes）

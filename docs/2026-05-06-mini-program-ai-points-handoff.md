...
账号落地先按“商家公司账号 -> 门店 -> 员工账号”三级理解：

- 公司账号是商家的结算主体，后续充值/服务包优先挂在公司或老板账号上。
- 门店账号用于区分不同门店的数据归属。
- 员工账号通过 `mp_account_memberships` 绑定到公司/门店，角色用于控制展示和权限。
- 当前后端已经有 `mp_companies`、`mp_stores`、`mp_account_memberships` 和 `profiles.account_role/company_id/store_id` 字段，但还没有管理入口。

初期定价可以按商家购买 token/AI 点包来做，不先做复杂订阅。当前扣点只覆盖小程序已经露出的页面能力；P1-P10 旧工作流不并入这一套。链接提取、分发、基础话术训练等低成本/引导型功能先保留 0 点，图片生成、正文生成、改写等明确消耗模型能力的动作扣点，视频生成等成本未稳定的能力先保留“按实际成本”。

...
2026-06-22 20:34 CST 追加：新增 `corepack pnpm aliyun:env:handoff` 和 `tests/aliyun-env-handoff.static.test.js`，把 APP production-cn 的 63 个环境变量整理成无值获取与导入手册。该手册按 `blockedRequired`、`appLaunchBlocking`、`readyPlainEnv`、`readySecretEnv` 和 `deferred` 分组，直接回答每个变量“从哪里取得、写到阿里云哪里、当前是否阻塞、禁止写到哪里”；它明确 `WECHAT_OPEN_APP_ID` 是服务端标识符只能导入 SAE plain env，`WECHAT_OPEN_APP_SECRET` 只能导入 KMS/Secrets Manager/SAE secret env，`APPLE_TEAM_ID` 是 Apple Developer 受控标识符且不能猜测。`aliyun:release:artifacts` 会随包输出 `env-handoff.json` 和 `env-handoff.md`；当前权威脚本口径为 `localPredeployChecks=48`、`predeployChecks=29`。该变更只增强本地操作包，不读取或输出任何真实 value，不创建阿里云资源、不导入环境变量、不部署。

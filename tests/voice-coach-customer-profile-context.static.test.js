...
test("customer turn prompting prioritizes explicit persona context over generic scenario defaults", () => {
  ...
  assert.match(sessionContextSource, /export function buildVoiceCoachFirstTurnTarget/)
  assert.match(sessionRouteSource, /buildVoiceCoachFirstTurnTarget\(sessionSnapshot, session\.id\)/)
  assert.match(sessionContextSource, /Open with one of the customer's explicit core concerns instead of a generic default objection\./)
  assert.match(sessionContextSource, /Past experience can reinforce the concern, but should not replace an explicit core concern when one exists\./)
})

...
test("Aliyun backend apply package separates immediate backend work from deferred app launch", () => {
  ...
  assert.ok(steps.get("BAP00_READONLY_INVENTORY_IDENTITY").userMustHandle.some((item) => item.includes("performance NAS usage-fee warning")))
  ...
  assert.match(steps.get("BAP00_READONLY_INVENTORY_IDENTITY").actionTimeConfirmation.minimumUserPhrase, /性能型 NAS/)
  assert.match(steps.get("BAP00_READONLY_INVENTORY_IDENTITY").actionTimeConfirmation.minimumUserPhrase, /只读盘点/)
  assert.ok(steps.get("BAP00_READONLY_INVENTORY_IDENTITY").actionTimeConfirmation.allowedActions.some((item) => item.includes("performance NAS usage-fee warning")))
  assert.ok(steps.get("BAP00_READONLY_INVENTORY_IDENTITY").actionTimeConfirmation.allowedActions.some((item) => item.includes("Reconnect the existing Aliyun CloudShell")))
  assert.ok(steps.get("BAP00_READONLY_INVENTORY_IDENTITY").actionTimeConfirmation.explicitlyExcluded.some((item) => item.includes("Purchase")))
  assert.ok(steps.get("BAP00_READONLY_INVENTORY_IDENTITY").actionTimeConfirmation.explicitlyExcluded.some((item) => item.includes("env import")))
...
test("Aliyun backend apply package markdown is value-free and actionable", () => {
  ...
  assert.match(markdown, /actionTimeConfirmation\.allowedActions: .*generated List\/Describe\/stat\/get inventory commands/)
  assert.match(markdown, /actionTimeConfirmation\.explicitlyExcluded: .*No Create\/Update\/Delete\/Deploy\/Start\/Stop\/Purchase\/DNS mutation commands/)

...
test("Aliyun backend apply package uses tracked fixtures for current blocked state", () => {
  ...
  assert.ok(steps.get("BAP01_RDS_POSTGRES_CREATE_AND_MIGRATE").currentEvidence.includes("appApiRoutesTouchingSupabaseCompatibility=54/56"))
...
test("Aliyun backend apply package reports field-level RDS blockers after local scaffold exists", () => {
  ...
  assert.ok(rdsStep.currentEvidence.includes("appApiRoutesTouchingSupabaseCompatibility=54/56"))
  assert.ok(rdsStep.currentEvidence.includes("appApiRoutesWithSupabaseDataAccess=4/56"))
...
test("Aliyun backend apply package markdown is value-free and current-state actionable", () => {
  ...
  assert.match(markdown, /appApiRoutesTouchingSupabaseCompatibility=54\/56/)

...
test("Aliyun APP account profile routes use RDS repository instead of mini-program profile re-export", () => {
  ...
  assert.match(appProfileRoute, /getAliyunRdsAppProfileContractResponse/)
  assert.match(appEntitlementsRoute, /getAliyunRdsAppEntitlementsResponse/)
  assert.doesNotMatch(appEntitlementsRoute, /@\/app\/api\/app\/profile\/route/)
  assert.doesNotMatch(appEntitlementsRoute, /getAliyunRdsAppProfile(?:Contract)?Response/)

...
test("Aliyun backend-cn status excludes WeChat mobile app from current backend blockers", () => {
  ...
  assert.equal(report.rdsMigration.appApiRoutesWithSupabase, 31)
  assert.equal(report.rdsMigration.appApiRoutesWithSupabaseDataAccess, 23)
  ...
  assert.equal(report.rdsMigration.firstVersionRdsRoutesWithSupabaseDataAccess, 19)

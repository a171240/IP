...
test("Aliyun blocker brief backend-only markdown stays focused on backend resources", () => {
  ...
  assert.match(markdown, /状态来源与一致性/)
...
test("APP production-cn backend-only current blocker brief is the active backend handoff", () => {
  ...
  assert.match(doc, /状态来源与一致性/)
  assert.match(doc, /corepack pnpm aliyun:backend-cn:status/)
  assert.match(doc, /corepack pnpm aliyun:sensitive:blockers:backend/)
  assert.match(doc, /corepack pnpm aliyun:blockers:brief:backend/)

...
test("APP production-cn backend-only sensitive docs reflect current Aliyun backend scope", () => {
  ...
  assert.match(sensitiveDoc, /## 后端-only 获取\/导入队列/)
  assert.match(sensitiveDoc, /queueScope: backend_aliyun_only/)
  assert.match(sensitiveDoc, /onlyMissingBackendCredentialValue: DATABASE_URL_CN/)
  assert.match(sensitiveDoc, /要回答的问题/)
  assert.match(sensitiveDoc, /DATABASE_URL_CN 从哪里获得并导入到哪里/)
  assert.match(sensitiveDoc, /阿里云控制台 -> RDS PostgreSQL/)

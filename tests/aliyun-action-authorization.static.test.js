...
test("Aliyun action authorization matrix separates local-safe work from external actions", () => {
  ...
  assert.equal(report.authorizationClosureBrief.blockedCredentialCount, 9)
...
test("Aliyun action authorization backend-only mode excludes deferred APP launch blockers", () => {
  ...
  assert.deepEqual(report.authorizationClosureBrief.blockedCredentialNames, ["ALIYUN_OSS_SECURITY_TOKEN", "DATABASE_URL_CN"])
  assert.equal(report.authorizationClosureBrief.blockedCredentialCount, 2)
...
test("Aliyun action authorization packet handoff documents the current packet gate", () => {
  ...
  assert.match(handoff, /blockedCredentialCount: 2/)
  assert.match(handoff, /blockedCredentialNames: ALIYUN_OSS_SECURITY_TOKEN, DATABASE_URL_CN/)

...
test("Aliyun cloud access report preserves current non-secret console evidence", () => {
  ...
  assert.match(resourcesObserved, /ACR Enterprise Economic cn-hangzhou 1 month purchase page visible, CNY 117\.00, instance name meiye-huajing, not purchased|ACR cn-hangzhou instances page visible; create enterprise instance button visible; .*purchase\/repository still action-time confirmation/)
  assert.match(resourcesObserved, /SAE console accessible; target app not proven created|SAE cn-hangzhou app list visible; .*target app meiye-huajing-app-api-production-cn not present/)

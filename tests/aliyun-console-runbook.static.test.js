...
test("Aliyun console runbook renders current console fields without secret values", () => {
  ...
  assert.equal(report.consoleClosureBrief.blockedCredentialCount, 9)
  assert.ok(report.consoleClosureBrief.blockedCredentialNames.includes("DATABASE_URL_CN"))
  ...
  assert.equal(acr.currentActionScope, "purchase_and_repository_only")
  assert.ok(acr.deferredActions.some((item) => item.includes("P04_ACR_IMAGE_AND_PULL")))
  assert.ok(acr.deferredActions.some((item) => item.includes("不执行 docker login/push")))
  ...
  assert.match(report.readyActionPackets[0].minimumAuthorizationPhrase, /不执行 docker login\/push/)
  assert.equal(report.readyActionPackets[0].currentActionScope, "purchase_and_repository_only")
  assert.ok(report.readyActionPackets[0].acceptanceEvidence.includes("acr.purchaseCandidate.confirmed=true"))
  assert.ok(report.readyActionPackets[0].acceptanceEvidence.includes("acr.registryHost actual aliyuncs.com host"))
  assert.ok(!report.readyActionPackets[0].acceptanceEvidence.includes("digestVerified=true"))
  assert.ok(report.readyActionPackets[0].deferredActions.some((item) => item.includes("imagePushed=true")))

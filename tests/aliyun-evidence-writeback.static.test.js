...
test("Aliyun evidence writeback backend-only mode excludes deferred APP launch gaps", () => {
  ...
  assert.equal(report.actionableWritebackSequence.packetCount, 7)
  ...
  assert.equal(packetById.get("P11_ALIYUN_RDS_DATA_MIGRATION").gapCount, 19)
  ...
  assert.equal(packetById.get("P04_ACR_IMAGE_AND_PULL").status, "can_start_after_action_time_confirmation")
  assert.equal(packetById.get("P04_ACR_IMAGE_AND_PULL").gapCount, 7)
  ...
  assert.match(schemaCompatibilityGap.expected, /compatibilityReviewChecklist 7 类/)
  assert.ok(schemaCompatibilityGap.requiredEvidence.some((item) => item.includes("compatibilityReviewChecklist 7 类")))
...
test("Aliyun evidence writeback checklist exposes local JSON write targets without secret values", () => {
  ...
  assert.ok(report.evidenceClosureBrief.partiallyObservedResourceEvidenceIds.includes("R02_ACR_IMAGE_REGISTRY"))
...
test("Aliyun image publish plan groups ACR and SAE writeback blockers by execution step", () => {
  ...
  assert.ok(report.summary.requiredAuthorizationPackets.includes("P04_ACR_IMAGE_AND_PULL"))
  assert.ok(report.summary.requiredAuthorizationPackets.includes("P08_SAE_RUNTIME_SLS"))
  assert.equal(report.summary.pushNetworkPathReady, false)
  assert.deepEqual(report.summary.recommendedPushNetworkPaths, [
    "vpc_registry_from_aliyun_network",
    "acr_import_task",
  ])
  assert.equal(report.summary.canStartP04AfterActionTimeConfirmation, true)
  assert.equal(report.summary.p04StrictReady, false)
  assert.equal(report.dockerContext.ok, true)
  assert.equal(report.dockerContext.status, "ready")
  assert.equal(report.dockerContext.checkedFiles, 7)
  assert.equal(report.dockerContext.sensitiveEnvExcluded, true)
  assert.equal(report.localDockerImage.status, "docker_daemon_unavailable_or_timeout")
  assert.equal(report.localDockerImage.dockerClientInstalled, true)
  assert.equal(report.localDockerImage.dockerServerAvailable, false)
  assert.match(report.localDockerImage.nextEvidenceAction, /ACR import\/VPC runner/)
  assert.equal(report.pushNetworkPlan.publicNetworkEntranceEnabled, false)
  assert.equal(report.pushNetworkPlan.selectedPath, "pending_choose_vpc_registry_or_enable_public_network_entrance")
  assert.equal(report.pushNetworkPlan.selectedReady, false)
  assert.deepEqual(report.pushNetworkPlan.selectedBlockers, ["acr.pushNetworkPath"])
  ...
  assert.equal(report.executionReadiness.selectedTransferPathReady, false)
  assert.equal(report.executionReadiness.localPublicPushReady, false)
  assert.equal(report.executionReadiness.dockerDaemonReady, false)
  assert.equal(report.executionReadiness.dockerClientInstalled, true)
  assert.equal(report.executionReadiness.canProceedWithoutLocalDockerDaemon, true)
  assert.equal(report.executionReadiness.nextOperatorDecision, "choose_vpc_registry_from_aliyun_network_or_acr_import_task")
  assert.deepEqual(report.executionReadiness.recommendedTransferPathIds, report.pushNetworkPlan.recommendedPathIds)
  assert.deepEqual(report.executionReadiness.forbiddenTransferPathIds, ["public_registry"])
  assert.ok(report.executionReadiness.postActionWritebackFields.includes("acr.remoteDigest=sha256:<64 hex>"))
  assert.ok(report.executionReadiness.postActionWritebackFields.includes("runtime.imagePullConfigured=true"))
  assert.ok(report.executionReadiness.verificationCommands.includes("corepack pnpm aliyun:image:plan:strict"))
  assert.ok(report.executionReadiness.safetyBoundary.some((item) => item.includes("Do not use public_registry")))
  ...
  const pushNetworkCandidates = new Map(report.pushNetworkPlan.candidates.map((item) => [item.id, item]))

  assert.equal(acrPurchase.ready, true)
  assert.equal(acrPurchase.canStartNow, false)
  assert.equal(acrPurchase.requiresActionTimeConfirmation, false)
  assert.ok(acrPurchase.requiredAuthorizationPackets.includes("P03_ACR_PURCHASE"))
  assert.deepEqual(acrPurchase.blockers, [])
  ...
  assert.equal(imagePush.canStartNow, true)
  ...
  assert.equal(imagePush.pushNetworkPlan.selectedReady, false)
  assert.deepEqual(imagePush.pushNetworkPlan.recommendedPathIds, report.pushNetworkPlan.recommendedPathIds)
  assert.equal(pushNetworkCandidates.get("public_registry").canUseNow, false)
  assert.ok(pushNetworkCandidates.get("public_registry").blockers.includes("acr.publicNetworkEntranceEnabled=false"))
  assert.equal(pushNetworkCandidates.get("public_registry").commandPreview, "blocked_until_public_registry_entrance_enabled_and_registry_host_ready")
  assert.equal(pushNetworkCandidates.get("vpc_registry_from_aliyun_network").canUseNow, true)
  assert.match(pushNetworkCandidates.get("vpc_registry_from_aliyun_network").remoteImage, /-vpc\.cn-hangzhou\.cr\.aliyuncs\.com/)
  assert.equal(pushNetworkCandidates.get("acr_import_task").canUseNow, true)
  assert.match(pushNetworkCandidates.get("acr_import_task").commandMode, /acr_import/)
  assert.ok(report.pushNetworkPlan.safetyBoundary.some((item) =>
    item.includes("Do not run local/public docker push while acr.publicNetworkEntranceEnabled=false"),
  ))
...
test("Aliyun cloud confirmations groups local evidence blockers by authorization packet", () => {
  ...
  assert.equal(oss.ossAccessPlan.selectedMode, "pending_choose_sae_runtime_role_or_sts")
  assert.equal(oss.ossAccessPlan.selectedReady, false)
  assert.deepEqual(oss.ossAccessPlan.selectedBlockers, ["oss.confirmed", "oss.ramLeastPrivilege"])
  assert.equal(oss.ossAccessPlan.executionReadiness.canStartP05AfterActionTimeConfirmation, true)
  assert.equal(oss.ossAccessPlan.executionReadiness.resourceReadyForP05, true)
  assert.equal(oss.ossAccessPlan.executionReadiness.accessGrantReady, false)
  assert.equal(oss.ossAccessPlan.executionReadiness.preferredModeId, "sae_runtime_role")
  assert.equal(oss.ossAccessPlan.executionReadiness.preferredModeAvoidsLongLivedSecret, true)
  assert.deepEqual(oss.ossAccessPlan.executionReadiness.fallbackSecretModeIds, [
    "sts_assume_role",
    "least_privilege_ram_user_secret_env",
  ])
  assert.ok(oss.ossAccessPlan.executionReadiness.postActionWritebackFields.includes("items.oss.confirmed=true"))
  assert.deepEqual(oss.ossAccessPlan.recommendedModeIds, ["sae_runtime_role", "sts_assume_role"])
  assert.equal(oss.ossAccessPlan.policyFile, "deploy/aliyun-production-cn.oss-ram-policy.json")
  assert.equal(oss.ossAccessPlan.policyName, "MeiyeHuajingServiceRecordsOssPolicy")
  assert.equal(oss.ossAccessPlan.writebackTemplate.jsonPath, "deploy/aliyun-production-cn.cloud-confirmations.local.json -> items.oss")
  assert.ok(oss.ossAccessPlan.writebackTemplate.optionalNonSecretFields.includes("credentialBoundary"))
  assert.ok(oss.ossAccessPlan.candidates.find((item) => item.id === "sae_runtime_role").writeBackFields.includes("items.oss.secretEnvNames=[]"))
  assert.ok(oss.ossAccessPlan.candidates.find((item) => item.id === "sts_assume_role").writeBackFields.includes("items.oss.secretEnvNames=ALIYUN_OSS_ACCESS_KEY_ID,ALIYUN_OSS_ACCESS_KEY_SECRET,ALIYUN_OSS_SECURITY_TOKEN"))
  assert.deepEqual(oss.ossAccessPlan.allowedActions, ["oss:GetObject", "oss:PutObject", "oss:PostObject"])
  assert.equal(
    oss.ossAccessPlan.resourceScope,
...
test("Aliyun evidence writeback markdown renders the same writeback boundaries", () => {
  ...
  assert.match(markdownOutput, /rdsMigrationGaps: 19/)
  ...
  assert.match(markdownOutput, /partiallyObservedResourceEvidenceIds: R02_ACR_IMAGE_REGISTRY, R05_OSS_AUDIO_STORAGE, R07_SLS_ALERTS/)
...
test("tracked APP production-cn evidence gap doc pins the current non-deployable writeback state", () => {
  ...
  assert.match(doc, /canStartNowPacketIds: P00_ALIYUN_READONLY_INVENTORY_IDENTITY, P11_ALIYUN_RDS_DATA_MIGRATION, P05_OSS_RAM_STS, P04_ACR_IMAGE_AND_PULL/)
  assert.match(doc, /blockedByDependencyPacketIds: P06_ENV_IMPORT, P07_DOMAIN_DNS_HTTPS, P08_SAE_RUNTIME_SLS/)
  ...
  assert.doesNotMatch(doc, /### P03_ACR_PURCHASE/)
  assert.match(doc, /P04_ACR_IMAGE_AND_PULL/)
  ...
  assert.match(doc, /partiallyObservedResourceEvidenceIds: R02_ACR_IMAGE_REGISTRY, R05_OSS_AUDIO_STORAGE, R07_SLS_ALERTS/)
  ...
  assert.match(fieldMap, /rdsMigrationGaps=19/)
  assert.match(fieldMap, /cloudInventoryResultGaps=1/)
  assert.match(fieldMap, /cloudConfirmationGaps=18/)
  assert.match(fieldMap, /imagePublishGaps=9/)
  assert.match(fieldMap, /Current scope is `backend_aliyun_only`/)
  assert.match(fieldMap, /Current field blockers: `18`/)
  assert.match(fieldMap, /Current field blockers: `9`/)
  assert.match(fieldMap, /Current field blockers: `19`/)
  assert.match(fieldMap, /`rdsPostgres\.databaseUrlCnSecretImported` \| `P11_ALIYUN_RDS_DATA_MIGRATION`/)
  assert.match(fieldMap, /`migration\.schemaCompatibilityReviewed` \| `P11_ALIYUN_RDS_DATA_MIGRATION`/)
  assert.match(fieldMap, /`migration\.supabaseSpecificSqlResolved` \| `P11_ALIYUN_RDS_DATA_MIGRATION`/)
  assert.match(fieldMap, /`migration\.rdsExtensionSupportConfirmed` \| `P11_ALIYUN_RDS_DATA_MIGRATION`/)
  ...
  assert.doesNotMatch(fieldMap, /`acr\.registryHost` \| `P03_ACR_PURCHASE`/)
...
test("tracked backend-only evidence writeback gap doc pins Aliyun backend scope", () => {
  const doc = read("docs", "app-production-cn-backend-evidence-writeback-gaps.md")
  const manifest = read("docs", "release-manifest-2026-06-21-app-aliyun-production-cn-bridge.md")

  assert.match(doc, /还不能部署阿里云后端/)
  assert.match(doc, /微信\/Android\/Apple 发布项已后置/)
  assert.match(doc, /currentScope: backend_aliyun_only/)
  assert.match(doc, /evidenceWritebackReady: 0\/4/)
  assert.match(doc, /totalGaps: 46/)
  assert.match(doc, /rdsMigrationGaps: 19/)
  assert.match(doc, /cloudInventoryResultGaps: 1/)
  assert.match(doc, /cloudConfirmationGaps: 18/)
  assert.match(doc, /imagePublishGaps: 8/)
  assert.match(doc, /blockedCredentialCount: 1/)
  assert.match(doc, /blockedCredentialNames: DATABASE_URL_CN/)
  assert.match(doc, /resourceEvidenceReady: 0\/7/)
  assert.match(doc, /canStartNowPacketIds: P00_ALIYUN_READONLY_INVENTORY_IDENTITY, P11_ALIYUN_RDS_DATA_MIGRATION, P05_OSS_RAM_STS, P04_ACR_IMAGE_AND_PULL/)
  ...
  assert.match(doc, /R07_SLS_ALERTS/)
  assert.match(doc, /partiallyObservedResourceEvidenceIds: R02_ACR_IMAGE_REGISTRY, R05_OSS_AUDIO_STORAGE, R07_SLS_ALERTS/)
  assert.match(doc, /DATABASE_URL_CN value/)
  assert.match(doc, /database password/)
  assert.match(doc, /AccessKeySecret/)
  assert.match(doc, /registry password/)
  assert.match(doc, /dump contents/)
  assert.match(doc, /customer data/)
  assert.match(doc, /corepack pnpm aliyun:rds:migration:evidence:strict/)
  assert.match(doc, /corepack pnpm aliyun:cloud:inventory-results:strict/)
  assert.match(doc, /corepack pnpm aliyun:cloud:confirmations:strict/)
  assert.match(doc, /corepack pnpm aliyun:image:plan:strict/)
  ...
  assert.doesNotMatch(doc, /WECHAT_OPEN_APP_ID/)
  assert.doesNotMatch(doc, /WECHAT_OPEN_APP_SECRET/)
  assert.match(manifest, /app-production-cn-backend-evidence-writeback-gaps\.md/)
  assert.match(manifest, /aliyun:evidence:writeback:backend/)
  assertNoSecretLikeValues(doc)
})

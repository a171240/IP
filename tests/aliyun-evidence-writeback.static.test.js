...
test("Aliyun evidence writeback backend-only mode excludes deferred APP launch gaps", () => {
  ...
  assert.equal(report.summary.rdsMigrationGaps, 17)
...
test("Aliyun evidence writeback markdown renders the same writeback boundaries", () => {
  ...
  assert.match(markdownOutput, /rdsMigrationGaps: 17/)
...
test("tracked APP production-cn evidence gap doc pins the current non-deployable writeback state", () => {
  ...
  assert.match(doc, /Production-cn cannot be deployed now\./)
  assert.match(doc, /evidenceWritebackReady=1\/4/)
  assert.match(doc, /totalGaps=56/)
  assert.match(doc, /rdsMigrationGaps=17/)
  assert.match(doc, /cloudInventoryResultGaps=0/)
  assert.match(doc, /cloudConfirmationGaps=27/)
  ...
  assert.match(doc, /do not replace `WECHAT_OPEN_APP_ID` or `WECHAT_OPEN_APP_SECRET`/)
  ...
  assert.match(doc, /Do not purchase ACR/)
  assert.match(doc, /Do not create or submit the WeChat Open Platform mobile app/)
  assert.match(doc, /Do not run docker login or docker push/)
  assert.match(doc, /Do not deploy production-cn/)
  assert.match(fieldMap, /Production-cn cannot be deployed now\./)
  assert.match(fieldMap, /evidenceWritebackReady=1\/4/)
  assert.match(fieldMap, /totalGaps=56/)
  assert.match(fieldMap, /rdsMigrationGaps=17/)
  assert.match(fieldMap, /cloudConfirmationGaps=27/)
  assert.match(fieldMap, /imagePublishGaps=12/)
  assert.match(fieldMap, /Current field blockers: `27`/)
  assert.match(fieldMap, /Current field blockers: `12`/)
  assert.match(fieldMap, /Current field blockers: `17`/)
  ...
  assert.match(fieldMap, /`items\.apiDomainHttps\.dnsResolvedToAliyun` \| `P07_DOMAIN_DNS_HTTPS_ICP`/)
  assert.match(fieldMap, /`items\.oss\.ramLeastPrivilege` \| `P05_OSS_RAM_STS`/)
  assert.match(fieldMap, /`items\.wechatOpenPlatform\.mobileAppSecretReady` \| `P01_WECHAT_OPEN_MOBILE_APP`/)
  assert.match(fieldMap, /`items\.wechatOpenPlatform\.androidSignature` \| `P10_ANDROID_RELEASE_SIGNING`/)
  assert.match(fieldMap, /`items\.wechatOpenPlatform\.iosConfigured` \| `P01_WECHAT_OPEN_MOBILE_APP`, `P02_APPLE_TEAM_ID`/)

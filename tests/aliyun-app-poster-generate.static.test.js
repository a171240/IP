...
test("APP poster generate persists a non-AI RDS draft bridge", () => {
  const route = read("app", "api", "app", "posters", "generate", "route.ts")
  ...
  assert.match(route, /action: "poster\.generate"/)
  assert.match(route, /kind: "poster"/)
  assert.match(route, /payload: body\.body/)
  assert.match(route, /App poster request was saved as a non-AI draft bridge/)
  assert.match(route, /Image generation, billing, OSS upload, saving, and sharing remain disabled/)
...
test("APP poster draft bridge keeps image generation, billing, OSS, and sharing disabled", () => {
  const route = read("app", "api", "app", "posters", "generate", "route.ts")
  const helper = read("lib", "aliyun-rds", "repositories", "app-content-workflows.server.ts")
  ...
  assert.doesNotMatch(combined, pattern(["generate", "Gpt", "Image", "2"]))
  assert.doesNotMatch(combined, pattern(["poster", "_", "generations"]))
  ...
  assert.doesNotMatch(combined, /\bshare[A-Za-z]*\(/)
})

...
test("Aliyun completion audit reports the current goal as blocked without secret values", () => {
  ...
  assert.deepEqual(report.goalClosureEvidenceBrief.credentialIntervention.blockedCredentialNames, ["ALIYUN_OSS_SECURITY_TOKEN"])
...
test("Aliyun completion audit carries console-only inventory evidence into G03 and markdown", () => {
  ...
  assert.match(markdownOutput, /blockedCredentialNames: ALIYUN_OSS_SECURITY_TOKEN/)

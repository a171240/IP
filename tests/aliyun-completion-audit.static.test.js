...
test("Aliyun completion audit reports the current goal as blocked without secret values", () => {
  ...
  assert.equal(report.summary.fullAppBlockedCredentialCount, 9)
  ...
  assert.equal(report.goalClosureEvidenceBrief.credentialIntervention.blockedCredentialCount, 2)
  assert.deepEqual(report.goalClosureEvidenceBrief.credentialIntervention.blockedCredentialNames, ["ALIYUN_OSS_SECURITY_TOKEN", "DATABASE_URL_CN"])
  ...
  assert.ok(byId.get("G09_SENSITIVE_BLOCKERS_EXPLICIT").evidence.includes("blockedCredentialCount=2"))
...
test("Aliyun completion audit carries console-only inventory evidence into G03 and markdown", () => {
  ...
  assert.match(markdownOutput, /blockedCredentialNames: ALIYUN_OSS_SECURITY_TOKEN, DATABASE_URL_CN/)

...
test("Aliyun RDS migration evidence command is wired into package scripts", () => {
  ...
  assert.equal(template.sourceInventory.appApiRoutesWithSupabaseDataAccess, 18)
  ...
  assert.equal(template.sourceInventory.firstVersionRdsRoutesWithSupabaseDataAccess, 14)
...
test("Aliyun RDS migration evidence init creates a non-secret local evidence scaffold", () => {
  ...
  assert.equal(local.sourceInventory.firstVersionRdsRoutesWithSupabaseDataAccess, 14)
...
test("Aliyun RDS migration evidence init does not overwrite an existing local evidence file", () => {
  ...
  const existing = {
    ...
    sourceInventory: {
      ...
      firstVersionRdsRoutesWithSupabaseDataAccess: 14,
...
test("Aliyun RDS migration evidence markdown is value-free", () => {
  ...
  assert.match(markdown, /firstVersionRdsRoutesWithSupabaseDataAccess: 14\/25/)

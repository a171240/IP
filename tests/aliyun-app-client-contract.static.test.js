...
test("APP client API contract audits implemented package-2 and content workflow facades", () => {
  ...
  assert.equal(report.auditedClientApiCalls, 61)
  assert.equal(report.uniqueAuditedClientRoutes, 55)
  assert.equal(report.deferredClientApiCalls, 0)
  assert.equal(report.matchedBackendRoutes, 46)
...
test("APP route and coverage gates include implemented package-2 facades", () => {
  ...
  assert.equal(routes.checkedRoutes, 51)
  assert.equal(routes.requiredRoutes, 33)
  assert.equal(routes.implementedFacadeRoutes, 18)
  ...
  assert.equal(coverage.checkedRoutes, 51)
  assert.equal(coverage.businessRoutes, 49)
  assert.equal(coverage.coverageOnlyProbes, 19)
  assert.equal(coverage.coverageProbes, 51)
  assert.equal(coverage.coveredBusinessRoutes, 49)

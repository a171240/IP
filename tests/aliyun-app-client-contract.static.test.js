...
test("APP route and coverage gates include implemented package-2 facades", () => {
  ...
  assert.equal(routes.checkedRoutes, 59)
  ...
  assert.equal(coverage.checkedRoutes, 59)
  assert.equal(coverage.businessRoutes, 57)
  assert.equal(coverage.coverageOnlyProbes, 28)
  assert.equal(coverage.coverageProbes, 60)
  assert.equal(coverage.coveredBusinessRoutes, 57)

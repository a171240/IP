...
test("Aliyun CloudShell handoff is wired into scripts, predeploy, deploy spec, and artifacts", () => {
  ...
  assert.match(readonlyInventoryDoc, /当前复核结论（2026-06-26）/)
  assert.match(readonlyInventoryDoc, /当前 strict inventory 不能视为 ready/)
  assert.match(readonlyInventoryDoc, /DRY_RUN_NOT_EXECUTED/)
  assert.match(readonlyInventoryDoc, /cloudInventoryStrictReady=false/)
  assert.match(readonlyInventoryDoc, /重启实例/)
  assert.match(readonlyInventoryDoc, /当前不可直接复用/)
  assert.match(readonlyInventoryDoc, /aliyun:cloudshell:collector:bootstrap \| pbcopy/)

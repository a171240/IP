...
test("Aliyun env handoff groups current variables without printing values", () => {
  ...
  const wechatAppId = report.groups.blockedRequired.find((item) => item.name === "WECHAT_OPEN_APP_ID")
  const wechatSecret = report.groups.blockedRequired.find((item) => item.name === "WECHAT_OPEN_APP_SECRET")

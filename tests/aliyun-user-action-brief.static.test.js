...
test("APP production-cn user action brief documents credential and operator handoff", () => {
  ...
  assert.match(doc, /blockedCredentialCount: 2/)
  assert.match(doc, /blockedCredentialNames: ALIYUN_OSS_SECURITY_TOKEN, DATABASE_URL_CN/)
  ...
  assert.doesNotMatch(doc, /WECHAT_OPEN_APP_ID|WECHAT_OPEN_APP_SECRET/)
  assert.doesNotMatch(doc, /MEIYE_RELEASE_STORE_PASSWORD|MEIYE_RELEASE_KEY_PASSWORD/)
  assert.doesNotMatch(doc, /U01_WECHAT_OPEN_APP_CREATE_AND_APPROVE|U10_ANDROID_RELEASE_SIGNING|U02_APPLE_TEAM_ID/)
})
...
test("Aliyun user action brief is value-free and includes the expected blockers", () => {
  ...
  assert.equal(report.credentialAcquisitionSummary.blockedCredentialCount, 9)
...
test("Aliyun user action brief backend-only mode excludes deferred APP launch blockers", () => {
  ...
  assert.deepEqual(report.summary.blockedCredentialNames, ["ALIYUN_OSS_SECURITY_TOKEN", "DATABASE_URL_CN"])
  assert.equal(report.credentialAcquisitionSummary.blockedCredentialCount, 2)
  assert.deepEqual(report.credentialAcquisitionSummary.blockedCredentialNames, ["ALIYUN_OSS_SECURITY_TOKEN", "DATABASE_URL_CN"])
  assert.deepEqual(report.summary.nextActionTimeConfirmations, [
    "P03_ACR_PURCHASE",
    "P05_OSS_RAM_STS",
    "P11_ALIYUN_RDS_DATA_MIGRATION",

...
test("Aliyun CLI inventory runner dry-run does not call cloud APIs or write raw output", () => {
  ...
  assert.equal(report.files.localResultsFile, localFile)
  ...
  assert.ok(local.operations.every((operation) => operation.evidence === "DRY_RUN_NOT_EXECUTED"))
...
test("Aliyun CLI inventory runner classifies missing CLI profile without storing raw stderr", () => {
  ...
  assert.match(report.localResults.operations[0].evidence, /^readonly_cli_I01_SAE_RUNTIME_/)
  assert.notEqual(report.localResults.operations[0].evidence, "DRY_RUN_NOT_EXECUTED")

...
test("image generation defaults stay single-image and support provider fallbacks", () => {
  ...
  assert.match(source, /function isTimeoutLikeMessage/)
  assert.match(source, /operation was aborted/)
  assert.match(source, /function isImageProviderTimeoutError/)
  assert.match(source, /image_provider_request_timeout/)
  assert.match(source, /if \(isImageProviderTimeoutError\(error\)\) return IMAGE_TIMEOUT_MESSAGE/)
  assert.match(source, /if \(isImageProviderTimeoutError\(error\)\) return 504/)
  assert.match(source, /function fallbackAfterMs/)
  assert.match(source, /APIMART_IMAGE_FALLBACK_AFTER_MS/)
  assert.match(source, /provider === "apimart" \? 150000 : 0/)
  assert.match(source, /image_provider_fallback_after_timeout/)
  assert.match(source, /code: "image_task_failed"/)
  assert.match(source, /fallbackUsed: failures\.length > 0/)
  assert.match(source, /providerElapsedMs: Date\.now\(\) - providerStarted/)
  assert.match(source, /totalElapsedMs: Date\.now\(\) - totalStarted/)
  assert.match(mpRoute, /model: typeof json\?\.model === "string" \? json\.model : ""/)
  assert.match(mpRoute, /fallbackUsed: json\?\.fallbackUsed === true/)
  assert.match(mpRoute, /providerFailureCount/)
  assert.match(mpRoute, /providerElapsedMs/)
  assert.match(mpRoute, /totalElapsedMs/)

    ...
    <header className="h-14 flex items-center justify-between px-6 dark:bg-zinc-950/80 bg-white/80 backdrop-blur-none md:backdrop-blur-xl border-b dark:border-white/10 border-black/[0.08] sticky top-0 z-40 transition-colors duration-300">
        ...
        {breadcrumbs.map((item, index) => (
          <div key={item.label} className="flex items-center gap-1.5">
            ...
            {item.href ? (
              <Link
                href={item.href}
                className="text-sm dark:text-zinc-500 text-zinc-500 dark:hover:text-zinc-300 hover:text-zinc-700 transition-colors"
              >
                {item.label}
              </Link>
            ) : (
              <span className="text-sm dark:text-white text-zinc-900 font-medium">{item.label}</span>
            )}
          </div>
        ))}
      </div>
      ...
      <div className="flex items-center gap-3">
        ...
        <div className="flex items-center gap-2 px-3 py-1.5 rounded-full bg-emerald-500/15 border border-emerald-500/30">
          ...
          <span className="text-[11px] text-emerald-400 font-medium tracking-wide">在线</span>
        </div>
      </div>
    </header>
  )

import type { VoiceCoachScenario } from "./scenarios.js"
import {
  calcFillerRatio,
  calcWpm,
  clampScore,
...
import { VoiceCoachReportSchema, type VoiceCoachReport } from "./report.js"
...
function weightedTotal(scores: {
  ...
  const total = scores.persuasion * 0.3 + scores.fluency * 0.2 + scores.expression * 0.2 + scores.pronunciation * 0.15 + scores.organization * 0.15
...
/**
 * Generate the final coaching report from persisted turn rows.
 */
export function generateVoiceCoachReport(opts: {
  ...
  for (const t of customerTurns) {
    const tag = (t as { features_json?: { tag?: unknown } }).features_json?.tag
  ...
  const firstPolished = String((firstBeautician?.analysis_json as { polished?: unknown } | undefined)?.polished || "").trim()
  const firstSuggestions = Array.isArray((firstBeautician?.analysis_json as { suggestions?: unknown } | undefined)?.suggestions)
    ? ((firstBeautician?.analysis_json as { suggestions?: unknown[] } | undefined)?.suggestions || [])
        ...
        submetrics: [
          {
            ...
            advice_paragraph: "建议用“三段式”组织：先共情与确认问题，再给信息与证据，最后给明确下一步（案例/体验/预约）。",
          },
        ],
        advice_paragraph:
